...
- An intersection (ej: Person Adult representing all the adult persons).
- A union (ex: Woman Girl representing women and girls).
- A complement (ex. -Woman representing all instances not of the class Woman).
- A minimal limitation of the number of instances in a class (ex: S3married to represents all the instances that are married to 3 or less persons).
- A maximal limitation of the number of instances in a class (ex: >2married to represents all the instances that are married to 2 or more people).
- A universal quantifier (ex: Vmarried To.Man indicates all the instances that are only married to men, one or more times). An existential quantifier (ex: 3 married To.Person indicates the entire instances that are married to one person, at least).
